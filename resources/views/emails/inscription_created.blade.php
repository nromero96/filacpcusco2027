...
<body>
    <div class="container">
        ...
        <p>Le informamos que su preinscripción para la <b>XIV CONGRESO BOLIVARIANO DE CIRUGIA PLASTICA FILACP 2027</b> ha sido recibida. La jornada se celebrará del 4 al 7 de mayo de 2027, en Valle Sagrado, Cusco - Perú.</p>
        ...
        <table>
            ...
            @endforeach
            @foreach($datainscription->tours as $tour)
            <tr><td>Tour: {{ $tour->name }} @if($tour->pivot->has_accompanist) <br><small>Acompañante: {{ $tour->pivot->accompanist_name }}</small> @endif</td><td>US$ {{ number_format($tour->pivot->unit_price + $tour->pivot->accompanist_price, 2) }}</td></tr>
            @endforeach
            <tr>
